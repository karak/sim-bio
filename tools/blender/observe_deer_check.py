"""assets/models/observe/deer.glb を Blender に読み戻して中身を数える (書き出しの検証)。

使い方: ~/.claude/skills/blender/scripts/run_blender.sh tools/blender/observe_deer_check.py -- assets/models/observe/deer.glb
出すもの: メッシュごとの三角形数 (材質別)、頂点色の有無、骨の数、アクションの長さ (30 fps)、材質と発光色、
寸法 (き甲の高さ・全高・向き)、1 頂点あたりの影響数、アクション中の root と骨盤の水平移動 (その場であること)。
"""
import json
import struct
import sys
...
path = argv[0] if argv else "assets/models/observe/deer.glb"
...
for o in sorted((o for o in scene.objects if o.type == "MESH" and o.name.startswith("deer")), key=lambda o: o.name):
    me = o.data
    by_mat = {}
    ...
    fur = {v for p in me.polygons if me.materials[p.material_index].name == "deer_body" for v in p.vertices}
    withers = max((pts[i].z for i in fur if -0.5 < pts[i].y < -0.3 and abs(pts[i].x) < 0.06 and pts[i].z < 1.6), default=0)  # 毛の背の線
    ...
    print(f"  bbox x {lo.x:.2f}..{hi.x:.2f}  y {lo.y:.2f}..{hi.y:.2f}  z {lo.z:.3f}..{hi.z:.2f}  withers z {withers:.3f}")
