...
- 原点は竜骨の底の中心で、船首は +Z(glTF)を向きます(変更なし)。
  外板の船体は 24.6 m、船首の巻きと船尾の柱まで入れて 26.6 m(試作 3 の渦巻きで 26.7 m から)、幅は 7.0 m(舷縁まで、舷縁の冠木を入れて 7.4 m)です(前は 14.1 m / 15.3 m / 3.9 m)。
  高さ(竜骨の底から): 中央の舷縁 5.6 m、主甲板 4.5 m、船尾楼の床 7.4 m・欄干の上 ≈ 9.7 m、船首の巻きの頂 ≈ 11.7 m(試作 3 の渦巻きで 11.2 m から)、帆柱の頂は前 18.8 m・主 22.0 m・後ろ 15.8 m。
...
|---|---:|---:|---|---|
| `ship_keel` | 764(508 から、試作 3 の渦巻き) | ≤ 12,000 | 5.44 × 26.56 × 12.69 | timber, block |
| `ship_ribs` | 3,736(3,480 から、試作 3 の渦巻き) | ≤ 12,000 | 7.18 × 26.56 × 12.24 | timber, block |
| `ship_planks` | 4,236(3,980 から、試作 3 の渦巻き) | ≤ 12,000 | 7.42 × 26.56 × 12.31 | timber, block, hull |
| `ship_mast` | 5,672(5,416 から、試作 3 の渦巻き) | ≤ 12,000 | 7.42 × 26.56 × 22.59 | timber, block, hull, deck |
| `ship_sails` | 10,604(10,348 から、試作 3 の渦巻き) | ≤ 12,000 | 10.40 × 27.89 × 22.59 | timber, block, hull, deck, rope, bell, bell_rim, sail |
| `ship_flying` | 11,037(10,781 から、試作 3 の渦巻き) | ≤ 12,000 | 11.85 × 27.89 × 24.60 | timber, hull, deck, block, rope, bell, bell_rim, sail, mu, mu_ring, lift |
...
|---|---:|---:|---:|---:|
| `belltree_mature` | 0.15(0.32 から) | 0.12〜0.15(0.23〜0.31 から) | 0.11〜0.15(0.22〜0.29 から) | 0.10〜0.14(0.22〜0.28 から) |
| `belltree_mature_lod1` | 0.16(0.34 から) | 0.11〜0.14(0.22〜0.30 から) | 0.09〜0.13(0.20〜0.29 から) | 0.08〜0.11(0.21〜0.25 から) |
| `forest_tree` | 0.15(0.22 から) | 0.11〜0.15(0.18〜0.23 から) | 0.11〜0.13(0.18〜0.21 から) | 0.09〜0.13(0.16〜0.21 から) |
| `forest_tree_lod1` | 0.14(0.19 から) | 0.09〜0.13(0.14〜0.19 から) | 0.09〜0.13(0.14〜0.18 から) | 0.08〜0.11(0.13〜0.17 から) |
...
## 追記(試作 3 の判断: 樹冠の房を詰める・船首の渦巻き)

2026-09-24 の試作 3 への判断(`docs/design/2026-09-23-observation-view.md`)を受けて、樹冠の房と船首を直しました。上の表の数は直した後の実測に更新しました(前の値は括弧内)。

- 樹冠の房: 引くと房が刈り込んだ木(ぽんぽん)に見えたので、房を大きくして隙間を詰め、光の筋が出る程度の狭い隙間だけ残しました。
  鐘樹(`CLUSTERS`)は下の輪の半径を 1.16〜1.26 m から 1.6〜1.72 m に、上の輪を 0.96〜1.0 m から 1.4〜1.45 m に上げ、下の輪を幹から 0.1 m 外へ、全体を 0.15〜0.25 m 上へ動かしました(上の輪は 0.1 m 内へ)。
  樹冠の差し渡しは 8.7 m から 9.9 m、高さは 9.5 m から 10.2 m になりました。三角形数・ノード名・原点・材質は同じで、鐘 30 個は房の下側に吊ったままです(lod0・lod1 とも 30 個)。
  森の木(`FOREST_CLUSTERS`)は半径を 0.96〜1.0 m から 1.16〜1.2 m に上げ、0.05〜0.15 m 上へ動かしました。lod1 の塊の拡大率は 1.08 から 1.02 に下げ、lod1 の隙間が lod0 より詰まりすぎないようにしました。
  日が抜ける割合は、鐘樹 0.08〜0.16、森の木 0.08〜0.15 になりました(目標 0.08〜0.15、一つにまとまっていた前の森の木は 0.03〜0.07)。真上から見ると、房の間に細い隙間が 5 本(森の木は 3〜4 本)残ります(`canopy-gaps.png`)。
- 船首の渦巻き: 四角い材の小さな鉤をやめ、船首の柱の先を大きな渦巻き(`observe_ship.py` の `volute`、半径 1.75 m から 0.4 m へ縮めて 1.5 巻き、目に丸く削った飾り)にしました。
  柱の傾きのまま前へ上がり、前の外周を上って頂で船尾側へ返り、内へ巻き込みます。竜骨の段から飛び立ちまで同じ形です(竜骨と一本の材なので、どの段にもあります)。
  前の支えと三角の帆は渦巻きの頂へ、船首の鐘の腕は渦巻きの最も前の点から出します(最も前の点へ縄を張ると縄が渦巻きを貫くため)。
  三角形は各段 +248〜256(`ship_flying` 11,037、予算 12,000 の内)。原点・向き・ノード名・材質・船体と帆柱の寸法は変えていません。
  全長(船首の巻きから船尾の柱まで)は 26.72 m から 26.56 m に、巻きの頂は ≈ 11.2 m から ≈ 11.7 m になりました。
- 描き直した図: `canopy-gaps.png`・`belltree.png`・`flora2.png`・`ship.png`・`ship-stages.png`・`shipyard.png`・`corner.png`(`corner.png` も成木が載るので描き直した)。

残る課題:

- 横から見た輪郭(`shipyard.png`・`flora2.png`)では、まだ房の丸みが一つずつ読めます。日の抜ける割合を保ったまま一つの樹冠に見せるには、房の縁の塊を横へ重ねる(上から見た隙間は変えない)か、Three.js 側の輪郭の揺らしで房の境目をぼかす手があります。
- 光の筋が本体で出るか(影の地図の解像度で 0.3〜0.5 m ほどに狭まった隙間が潰れないか)は、Three.js の画面で確かめる必要があります。
- 渦巻きは 4 角の材なので、寄ると角ばりが見えます。基準画の彫り物(渦の溝)はありません。
