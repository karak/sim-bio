...
## Acceptance criteria (v2: キーアイテムで作り直し、2026-09-21)

- [x] 燃料の蓄え(上限 4 年分)と負債(3 年分で衰退)、`start.civilization.fuelStock`。単体テスト `tests/unit/civilizationFuel.test.ts`、`tests/unit/world.civilization.fuel.test.ts`
- [x] 炎蜥蜴は熱の門(`minHeat`)で湧き、噴火後に半径 5 の鹿が 5 割を割り、熱が冷めれば(8 年)消える。`tests/unit/firelizard.test.ts`
- [x] 鐘樹の陰(`shade 0.2`)の代償と放流の定着。`tests/unit/belltree.test.ts`
- [x] tests/slow: 塔の重さ v2 で放置・火だけ・樹だけは dead、配分 2 通りは alive。既存 15 件も通る(20 件)
- [x] HUD の燃料表示「蓄え / 必要年」と火の山の案内。`tests/e2e/smoke.spec.ts`
- [x] 設計書 §4.15 と §6、LD 文書 §7/§8 に係数・行列・捨てた案
...
- 判定行列の分離までの試行は LD 文書 §7(捨てた案)と §8(校正で足した仕組み)、係数は設計書 §4.15。要点: 燃料の蓄えと負債、炎蜥蜴の熱の門(`minHeat`、`HEAT_SEED`、`HEAT_FLOOR 0`)、鐘樹 `shade 0.2`、火口 3167・集落 2847、予算(噴火 24・年収 12)。
- 炎蜥蜴の副作用テストは新設計に合わせて「噴火 8 年後にピークの 1 割未満」に更新(熱が 1 を割っても門が 0.3 程度ある間は居着く。再測定はテストの docstring)。
- HUD の火の山の案内を「島の印(火口)に打てば」に修正(v2 では火口が最高地点ではない)。
- 検証: `npx tsc --noEmit`、`npx eslint .`、`npx vitest run`(212 件)、`npx playwright test`(15 件)通過。`SLOW=1 npx vitest run tests/slow`(20 件)通過。夜間はバックグラウンドの vitest が CPU を 2% しか貰えず 600 秒のタイムアウトで偽失敗したので、`caffeinate -dimsu` を付けて前面で 3 分割(`-t` で絞る)して回した。
- 通し実行で配分 2 通りが dead(想定解 1 は 19 年目に民の不足で衰退、想定解 2 は鹿 13.0 < 13.8)。ヘッドレスの行列は `heatSpecies` の修正前(全種が熱で湧くバグ)の結果だった。原因は 2 つ: 火口 3167 が集落から 5 セルで噴火の焼け跡が支え半径と重なること、鐘樹が `diffusion 0.005` で島中に広がり陰で鹿が半減すること(炎蜥蜴の捕食を 0 にしても鹿の平均は変わらない)。火口を 3223(10 セル)に、鐘樹の `diffusion` を 0.002 に。開始時の蓄え 45 が上限 24 に切り捨てられていたのも直した(放置は 11 年目から落ちる)。
